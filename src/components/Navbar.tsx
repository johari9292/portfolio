...
export default function Navbar() {
  ...
  return (
    <motion.nav
      initial={{ y: -100 }}
      animate={{ y: 0 }}
      className="fixed w-full bg-white/80 dark:bg-dark/80 backdrop-blur-sm z-50 shadow-sm"
    >
      ...
      {isOpen && (
        <motion.div
          ...
          className="md:hidden"
        >
          <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3 bg-white dark:bg-dark">
            {navItems.map((item) => (
              <motion.div
                key={item.name}
